...
A small native CLI and TUI for importing and preparing books for CrossPoint readers. It supports
downloaded Kobo store books and sideloaded EPUBs from a mounted Kobo, with local EPUB output.
It also fulfills Adobe/ByteBooks ACSM files through bundled libgourou, using an
existing activation export. Python and Calibre are not required at runtime.
